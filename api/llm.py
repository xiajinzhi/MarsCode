...
class LLMManager:
    def __init__(self, config: Any, prompts: Dict[str, str]):
        ...
        if self.llm_type == "ANTHROPIC_API":
            ...
            openai.base_url = config.llm.url  
            openai.api_key = config.llm.key
    ...
    def get_text(self, messages: List[Dict[str, str]], stream: Optional[bool] = None) -> Generator[str, None, None]:
        ...
        try:
            if self.llm_type == "OPENAI_API" or self.llm_type == "ARK_API":
            ...
            elif self.llm_type == "HF_API":
                print("run llm HF_API")
                yield from self._get_text_hfapi(messages, stream)
        except Exception as e:
            raise APIError(f"LLM Get Text Error: Unexpected error: {e}")
        

    def _get_text_hfapi(self, messages: List[Dict[str, str]], stream: bool) -> Generator[str, None, None]:
        """
        ...
        print("llm json:")
        print(messages)
        response = requests.post(url=self.config.llm.url, json={'messages': messages})
        
        def truncate_string(input_string):
            # 将字符串转换为小写
            lower_string = input_string.lower()
            # 查找"code:"的位置（不区分大小写）
            code_index = lower_string.find("code:")
            if code_index != -1:
                # 如果找到了，截取"code:"之前的字符串
                return input_string[:code_index]
            else:
                # 如果没有找到，返回原始字符串
                return input_string
        
        yield truncate_string(response.json().get("generated_text", ""))
    ...
    def _get_text_openai(self, messages: List[Dict[str, str]], stream: bool) -> Generator[str, None, None]:
        ...
        headers = {
        "Authorization": f"Bearer {self.config.llm.key}",
        "Content-Type": "application/json"
        }

        # 构建请求数据
        data = {
            "model": f"{self.config.llm.name}",  # 指定模型，例如 gpt-3.5-turbo
            'messages': messages,
