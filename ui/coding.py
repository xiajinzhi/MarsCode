...
def send_request(
    ...
    while has_text_item or has_audio_item:
        ...
        print("coding.py text_chunks:")
        print("".join(text_chunks))

        # yield chat_history, chat_display, code, audio_chunk
        yield chat_history, chat_display, code, audio_chunk, "".join(text_chunks)
...
def get_problem_solving_ui(
    ...
    # send_request_partial = partial(send_request, llm=llm, tts=tts, send_request=True)
    send_request_partial = partial(send_request, llm=llm, tts=tts, silent=True)
    ...
    with gr.Tab("Interview", render=False, elem_id=f"tab") as problem_tab:
        ...
        with gr.Accordion("Solution", open=True, visible=False) as solution_acc:
            with gr.Row() as content:
                ...
                with gr.Column(scale=1):
                    ...
                    #
                    new_audio_params = default_audio_params
                    new_audio_params["streaming"] = False
                    audio_input = gr.Audio(interactive=False, **new_audio_params, elem_id=f"audio_input")
                    ...
                    ######
                    # 创建一个按钮来触发音频处理
                    process_btn = gr.Button("Process Audio", elem_id="process_btn")
                    reply = gr.Textbox(label="reply", visible=False)
        ...
        process_btn.click(
            # 步骤1：将音频文件转为文本
            fn=stt.process_audio_save_file,
            inputs=[audio_input, hidden_text],
            outputs=[hidden_text]
        ).success(
            # 步骤2：将文本添加到聊天历史
            fn=stt.add_to_chat,
            inputs=[hidden_text, chat],
            outputs=[chat]
        ).success(
            # 步骤3：设置转录状态为 False（表示转录完成）
            fn=lambda: False,
            outputs=[is_transcribing]
        ).success(
            # 步骤4：发送 LLM 请求并获取响应
            fn=send_request_partial,
            inputs=[code, previous_code, chat_history, chat],
            outputs=[chat_history, chat, previous_code, audio_output, reply],
            show_progress="full"
        ).then(
            # 步骤5：用 TTS 播放 LLM 的回复（可选）
            fn=tts.read_chat_message,
            inputs=[reply],
            outputs=[audio_output]
        ).then(
            # 步骤6：重置缓冲区、隐藏文本和转录状态
            fn=lambda: (np.array([], dtype=np.int16), "", False),
            outputs=[audio_buffer, hidden_text, is_transcribing]
        ).then(
            # 步骤7：重新启用音频输入控件
            fn=lambda: gr.update(visible=True),
            outputs=[audio_input]
